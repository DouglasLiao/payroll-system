...
- `POST /api/auth/update-timeout/` - Configurar timeout
...
- `POST /api/companies/:id/create-admin/` - Criar Customer Admin
- `GET /api/companies/:id/admins/` - Listar admins da empresa
- `GET /api/companies/:id/providers/` - Listar providers da empresa
...
- ✅ `authApi.ts` - API service com suporte a cookies (`withCredentials`)
...
- `api/models.py` - Modelos (Company, User, Provider)
- `api/permissions.py` - Permissions e decorators
- `api/auth_views.py` - Endpoints de autenticação
- `api/company_views.py` - Gerenciamento de empresas
- `api/protected_views.py` - ViewSets protegidos (para aplicar)
...
- `src/services/authApi.ts` - API service
