...
- `GET /api/providers/{id}/` - Detalhe do prestador
- `PUT /api/providers/{id}/` - Atualizar prestador
- `DELETE /api/providers/{id}/` - Excluir prestador

### Payrolls (Folhas de Pagamento)
- `GET /api/payrolls/` - Listar folhas
- **`POST /api/payrolls/calculate/`** - Criar folha (com cálculo automático)
- `GET /api/payrolls/{id}/` - Detalhe da folha (com itens)
- `POST /api/payrolls/{id}/close/` - Fechar folha
- `POST /api/payrolls/{id}/mark-paid/` - Marcar como paga
- `PUT /api/payrolls/{id}/recalculate/` - Recalcular folha DRAFT
- `POST /api/payrolls/{id}/reopen/` - Reabrir folha fechada
- `DELETE /api/payrolls/{id}/` - Excluir folha DRAFT

### Filtros
Todos os endpoints de listagem suportam filtros via query parameters:
```
GET /api/payrolls/?status=DRAFT
GET /api/payrolls/?reference_month=01/2026
GET /api/payrolls/?provider=1
```
