...
class PayrollService:
    ...
    def create_payroll(
        ...
        if Payroll.objects.filter(provider=provider, reference_month=reference_month).exists():
            raise ValueError(
    ...
    def _create_payroll_items(self, payroll: Payroll) -> None:
        ...
        if payroll.advance_value > 0:
            percentage = (payroll.advance_value / payroll.base_value * 100).quantize(Decimal('0.01'))
            items.append(PayrollItem(
                payroll=payroll,
                type=ItemType.DEBIT,
    ...
    def get_payroll_details(self, payroll_id: int) -> Dict:
        ...
        items = PayrollItem.objects.filter(payroll=payroll).order_by('type', 'description')
