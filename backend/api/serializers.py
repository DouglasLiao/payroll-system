...
class PayrollCreateSerializer(serializers.Serializer):
    ...
    **Exemplo de Response (201 Created):**
    ...
    provider_id = serializers.IntegerField(
        help_text="ID do prestador"
    )
    reference_month = serializers.CharField(
        max_length=7,
        help_text="Mês de referência (formato: MM/YYYY, ex: 01/2026)"
    )
    ...
    notes = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Observações sobre esta folha"
    )
    ...
    def validate(self, data):
        ...
        for field in ['overtime_hours_50', 'holiday_hours', 'night_hours', 'absence_hours']:
        ...
        if data.get('late_minutes', 0) < 0:
            raise serializers.ValidationError({
                'late_minutes': "Não pode ser negativo"
            })
