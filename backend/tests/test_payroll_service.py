...
def test_create_payroll():
    ...
    assert payroll.hourly_rate == Decimal('10.00'), f"Valor/hora incorreto: {payroll.hourly_rate}"
    assert payroll.advance_value == Decimal('880.00'), f"Adiantamento incorreto: {payroll.advance_value}"
    assert payroll.remaining_value == Decimal('1320.00'), f"Saldo incorreto: {payroll.remaining_value}"
    assert payroll.overtime_amount == Decimal('150.00'), f"Hora extra incorreta: {payroll.overtime_amount}"
    assert payroll.holiday_amount == Decimal('160.00'), f"Feriado incorreto: {payroll.holiday_amount}"
    assert payroll.night_shift_amount == Decimal('40.00'), f"Noturno incorreto: {payroll.night_shift_amount}"
    assert payroll.dsr_amount == Decimal('25.00'), f"DSR incorreto: {payroll.dsr_amount}"
    assert payroll.late_discount == Decimal('5.00'), f"Desconto atraso incorreto: {payroll.late_discount}"
    assert payroll.absence_discount == Decimal('80.00'), f"Desconto falta incorreto: {payroll.absence_discount}"
    assert payroll.dsr_on_absences == Decimal('13.33'), f"DSR s/ faltas incorreto: {payroll.dsr_on_absences}"
    assert payroll.total_earnings == Decimal('1695.00'), f"Total proventos incorreto: {payroll.total_earnings}"
    assert payroll.total_discounts == Decimal('300.73'), f"Total descontos incorreto: {payroll.total_discounts}"
    assert payroll.net_value == Decimal('1394.27'),f"Valor líquido incorreto: {payroll.net_value}"
...
def test_recalculate_payroll():
    ...
    assert updated_payroll.overtime_hours_50 == Decimal('15'), "Horas extras não atualizadas"
    assert updated_payroll.holiday_hours == Decimal('8'), "Horas feriado não atualizadas"
