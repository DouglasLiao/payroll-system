...
class ProviderAdmin(admin.ModelAdmin):
    list_display = ['name', 'role', 'monthly_value', 'monthly_hours', 'advance_enabled', 'payment_method']
    ...
    fieldsets = (
        ...
        ('Configurações Contratuais PJ', {
            'fields': ('monthly_value', 'monthly_hours', 'advance_enabled', 'advance_percentage', 'vt_value')
        }),
        ('Dados de Pagamento', {
            'fields': ('payment_method', 'pix_key', 'bank_name', 'bank_agency', 'bank_account')
        }),
    )
...
class PaymentAdmin(admin.ModelAdmin):
    list_display = ['provider', 'reference', 'amount_base', 'total_calculated', 'status', 'paid_at']
...
class PayrollAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Identificação', {
            'fields': ('provider', 'reference_month', 'status')
        }),
        ('Valores Base', {
            'fields': ('base_value', 'hourly_rate', 'advance_value', 'remaining_value')
        }),
        ('Horas Trabalhadas', {
        ...
        ('Descontos Variáveis', {
            'fields': ('late_minutes', 'absence_hours', 'manual_discounts', 'vt_discount')
        }),
