...
export const CashFlowChart = ({
  ...
  // Get last 6 months from actual data
  const selectedMonths = getLastNMonthsFromData(monthlyData, 6)

  // Calculate income (paid payrolls) and expenses (closed payrolls)
  const incomeData = selectedMonths.map((m) => monthlyData[m]?.paid.value || 0)
  const expenseData = selectedMonths.map(
    (m) => monthlyData[m]?.closed.value || 0
  )

  // Calculate accumulated balance using reduce to avoid mutation
  const balanceData = selectedMonths.reduce<number[]>((acc, m) => {
    const income = monthlyData[m]?.paid.value || 0
    const expense = monthlyData[m]?.closed.value || 0
