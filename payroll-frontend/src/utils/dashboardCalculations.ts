...
/**
 * Get list of last N months from actual data in MM/YYYY format
 * Returns months in chronological order (oldest to newest)
 * This ensures we always show months that have data
 */
export const getLastNMonthsFromData = (
  monthlyData: MonthlyData | Record<string, unknown>,
  n: number
): string[] => {
  const allMonths = Object.keys(monthlyData)
  ...
  // Sort chronologically and take the last N months
  const sortedMonths = sortMonthsChronologically(allMonths)
  return sortedMonths.slice(-n)
}
